...
#     axs[1].plot(x_vals_sweep, len(x_vals_sweep)*[90],'--', c=case['colour'])
#     axs[2].plot(x_vals_sweep, mus_90, '--', c=case['colour'])
#
# pd_ax = axs[0]
# if relative_change:
...
Eg_start = 0.062
y_sweep = np.linspace(Eg_start, 0.15, 20)
y_id_str = 'Eg'
args_to_opt= ['mu']
arg_fix_extra = {'eta_ext':1, 'consider_nonrad':False}
...
filename = f'PD_cwv{cwv}_cutoffangle_10_90_21_Egs_{Eg_start}_02_15.csv'
...
# pds_opt = []
for i in range(10):
    opt_xs, opt_pd = get_best_pd(combined_trd_env, args_to_opt=args_to_opt+[x_id_str,y_id_str], args_to_fix=arg_fix_extra, alg = alg)
